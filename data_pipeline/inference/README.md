...
- `gemini_backend.py` uses Vertex AI through `google-genai`; install it with `bash scripts/setup_env.sh data-gemini`.
- `vllm_backend.py` uses vLLM and `qwen-vl-utils`; install it with `bash scripts/setup_env.sh data-vllm` on a compatible GPU host.
